...
public class Sp_Manga_enquete {
	...
	public static Boolean execute(WebDriver driver, AccountBean bean) throws Exception {
		...
		for (int i = 0; i < 10; i++) {
			...
			try{
				// 「次へ」
				driver.findElement(By.xpath("//input[@alt='次へ']")).click();
				// 「タブ閉じる」
				tab_close(driver, originalHandle, By.xpath("//input[@alt='次へ']"));
			}catch (Exception e1) {
				scroll(driver);
				try{
					// 「次へ」
		...
		try{
			// 「クリックしてポイントゲット」
			driver.findElement(By.xpath("//input[@alt='クリックしてスタンプゲット']")).click();
			// 「タブ閉じる」
			tab_close(driver, originalHandle, By.xpath("//input[@alt='クリックしてスタンプゲット']"));
		}catch (Exception e1) {
			scroll(driver);
			try{
				// 「クリックしてポイントゲット」
	...
	public static String get_content(WebDriver driver) {
		String text03 = "";
		try {
			text03 = driver.findElement(By.className("enquete_number")).getText();
			return text03;
		} catch (Exception e) {
		}
		return text03;
